...
{
    /// <summary>
    /// Sketchy-Ass-Class, Please use wisely.
    /// </summary>
    public class Hash
    {
        ...
        public static string PasswordHASH(string str)
        {
            ...
            {
                for (int index4 = 0; index4 < charArray.Length; ++index4)
                {
                    if (index2 > length - 1)
                        index2 = 0;
                    int num4 = (int)(byte)charArray[index4] ^ num3;
            ...
            for (int index3 = 0; index3 < charArray.Length; ++index3)
            {
                if (index5 > length - 1)
                    index5 = 0;
                int num4 = (int)(byte)((int)charArray[index3] ^ num5 ^ num3);
        ...
        public static int GHash(byte[] @byte)
        {
            int num1 = 23;
            int num2 = 0;
            foreach (byte num3 in @byte)
            {
                ++num2;
                num1 += num2 | (int)num3;
            }
            return num1 * @byte.Length;
        }

        public static int getCRC(byte[] @byte)
        {
            return Hash.GHash(@byte);
        }
    }
}
